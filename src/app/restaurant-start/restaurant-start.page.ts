...
@Component({
  ...
  templateUrl: './restaurant-start.page.html',
  styleUrls: ['./restaurant-start.page.scss'],
})
export class RestaurantStartPage implements OnInit {
  ...
  applyEvent() {
    this.navCtrl.navigateForward('restaurant-success/restaurant-menu');
  }
  ...
  async submitPhoto(file: File){
    ...
    this.necEvaService.postPictureAndGetPerson(file,'restaurant-ng').pipe(take(1)).subscribe(ref =>{
      console.log(ref);
      loading.dismiss();
      if(ref.firstName){
        ...
        this.navCtrl.navigateForward('restaurant-ng');
      }
    }); 
  }
